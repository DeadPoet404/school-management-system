...
// ── Raw API row shapes (string | number for safety) ────────────────────
...
// ── Empty fallbacks (used while loading / on partial data) ─────────────
...
async function fetchFinancePage<T>(
  ...
  // NOTE: fetchWithAuth(url: string) — plain function call with a template
  // literal argument (NOT a tagged template). It prepends API_URL (/api),
  // so FINANCE_BASE_PATH="/finance" → final URL "/api/finance/<resource>".
...
function mapPayrollRecord(row: PayrollApiRow): PayrollRecord {
  ...
  const allow = toNumber(row.allowances);
  const ded = toNumber(row.deductions);
  return {
    id: row.id,
    staffName: row.name,
    baseSalary: base,
...
function normalizeTrendPoint(
  point: Partial<FinanceDashboardTrendPoint>,
...
function normalizeDashboardData(
  data: Partial<FinanceDashboardData> | undefined,
  days: number,
): FinanceDashboardData {
  const trend = Array.isArray(data?.trend)
    ? data!.trend.map(normalizeTrendPoint)
    : [];
...
export function useFinanceDashboard(days: number = 90) {
  return useQuery<FinanceDashboardData>({
    ...
    queryFn: async () => {
      ...
      // Backend dashboard endpoint lands in Phase 1; degrade gracefully
      // to empty data on 404 so the page doesn't crash on Phase-0 builds.
      const res = await fetchWithAuth(`${FINANCE_BASE_PATH}/dashboard?${params}`);
      if (res.status === 404) {
        return normalizeDashboardData(undefined, days);
      }
...
export function usePayroll(page: number = 1, limit: number = 20) {
  return useQuery<PaginatedResult<PayrollRecord>>({
...
export function useCollections(page: number = 1, limit: number = 20) {
  return useQuery<PaginatedResult<CollectionRecord>>({
    queryKey: ["finance", "collections", page, limit],
    queryFn: async () => {
      const result = await fetchFinancePage<CollectionApiRow>(
        "collections",
        page,
        limit,
      );
...
export function useInvoices(page: number = 1, limit: number = 20) {
  return useQuery<PaginatedResult<InvoiceRecord>>({
    queryKey: ["finance", "invoices", page, limit],
    queryFn: async () => {
      const result = await fetchFinancePage<InvoiceApiRow>(
        "invoices",
        page,
        limit,
      );
...
export function useExpenses(page: number = 1, limit: number = 20) {
  return useQuery<PaginatedResult<ExpenseRecord>>({
    queryKey: ["finance", "expenses", page, limit],
    queryFn: async () => {
      const result = await fetchFinancePage<ExpenseApiRow>(
        "expenses",
        page,
        limit,
      );
